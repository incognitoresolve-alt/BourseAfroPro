...
- **Format des échanges API** : `/api/*` échange du [TOON](https://toonformat.dev) (`Content-Type: text/toon`) plutôt que du JSON — plus compact à volume de données égal. Encodage/décodage via `@toon-format/toon`, côté serveur (`functions/lib/toon.ts`) et côté client (`src/lib/market-api.ts`, `src/lib/portfolio-api.ts`).
- **Simulateur** (`/simulateur`) : entièrement fonctionnel — cours BRVM en direct, portefeuille réel, achat/vente exécutés via `/api/portfolio`. Nécessite une connexion (Supabase Auth).
